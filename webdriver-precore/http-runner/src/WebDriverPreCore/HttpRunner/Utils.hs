...
callWebDriver' endpoint logger request =
  parseResult <$> callWebDriverBody' endpoint logger request
...
callWebDriverBody' endpoint logger =
  fmap (.body) . runHttpRequest endpoint logger
...
callWebDriverResponse' ::
  ...
  HttpRequest ->
  m HttpResponse
callWebDriverResponse' endpoint logger request =
  runHttpRequest endpoint logger request
...
runHttpRequest ::
  ...
  HttpRequest ->
  m HttpResponse
runHttpRequest MkHttpEndpoint {host, port} logger request = do
  logger $ "HTTP " <> methodText request.method <> " " <> pack (show url)
  response <- runReq defaultHttpConfig {httpConfigCheckResponse = \_ _ _ -> Nothing} $ do
    r <- case request.method of
      GET_METHOD -> req GET url NoReqBody jsonResponse (R.port iPort)
      POST_METHOD -> req POST url (ReqBodyJson $ fromMaybe (object []) request.body) jsonResponse (R.port iPort)
      DELETE_METHOD -> req DELETE url NoReqBody jsonResponse (R.port iPort)
    pure
      MkHttpResponse
  ...
  logger $ "Response: " <> pack (show response.statusCode)
  pure response
  where
    iPort = fromIntegral port
    url = buildUrl (http host) request.path
    methodText = \case
      GET_METHOD -> "GET"
      POST_METHOD -> "POST"
