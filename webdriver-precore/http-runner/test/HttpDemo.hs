{-|
HTTP Demo module for webdriver-precore-http-runner tests
...
import HttpDemoUtils (HttpDemo, demo, runDemo, sessionDemo)
...
import WebDriverPreCore.Test.TestServerAPI (testServerHomeUrl, withTestServer)
...
demoWindowHandles =
  ...
  where
    ...
    action sesId MkDemoActions {..} MkHttpActions {..} = do
      ...
      let HandleSpec {handle} = w
      switchToWindow sesId handle
