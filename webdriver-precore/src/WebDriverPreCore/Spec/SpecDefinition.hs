...
--
--
-- This is the spec layer
--
--
setTimeouts :: SessionId -> Timeouts -> W3Spec Timeouts ()
setTimeouts sessionRef timeouts =
  Post "Set Timeouts" (sessionUri1 sessionRef "timeouts") timeouts

getTimeouts :: SessionId -> W3Spec NoBody Timeouts
getTimeouts sessionRef = Get "Get Timeouts" (sessionUri1 sessionRef "timeouts")

getTitle :: SessionId -> W3Spec NoBody Text
...
class Serialize b where
    encode :: b -> ByteString
class Deserialize b where
    decode :: ByteString -> b

instance Serialize NoBody where
    encode _ = ""

instance Deserialize Timeouts where
     decode b = undefined

class (Monad m) => HttpClient m where 
    get :: Text -> m ByteString
...
getResponse spec = case spec of
    Get {description, path} -> get $ fullPath path
...
--
--
-- This is an IO layer / impl
--
--
newtype Wreq a = Wreq { unWreq :: IO a } deriving (Functor, Applicative, Monad)
instance HttpClient Wreq where
    get path = undefined -- MkReq path
...
--
--
instance (Show a) => Show (W3Spec a b) where
