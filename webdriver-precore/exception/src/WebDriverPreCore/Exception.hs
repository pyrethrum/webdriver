{-|
Module: WebDriverPreCore.Exception
Description: Exception types and helpers for WebDriver

This module provides exception types and helper functions for handling 
WebDriver errors in both HTTP and BiDi protocols.
-}
module WebDriverPreCore.Exception
  ( -- * Exception Types
    WebDriverException (..),
    ...
    tryWebDriverException,
    throwWebDriverException,
    isProtocolException,
    isParseException,
    getErrorType,
  )
...
import Control.Exception (Exception (..), catch, handle, try, throw)
...
-- | A JSON-safe unsigned integer type (duplicated for decoupling)
newtype JSUInt = MkJSUInt Word64 
  deriving newtype (Show, Eq, Enum, FromJSON)

-- | JSON text conversion helper
jsonToText :: Value -> Text
jsonToText = pack . show
...
-- | Parse a WebDriver exception from a JSON response
...
parseWebDriverException errInfo response =
  ...
  where
    parserErr msg = ResponseParseException {message = msg, response}
...
-- | Parse just the error type from a response, if present
...
-- | Catch a 'WebDriverException'
catchWebDriverException :: IO a -> (WebDriverException -> IO a) -> IO a
catchWebDriverException = catch

-- | Handle a 'WebDriverException'  
handleWebDriverException :: (WebDriverException -> IO a) -> IO a -> IO a
handleWebDriverException = handle

-- | Try an action that may throw a 'WebDriverException'
tryWebDriverException :: IO a -> IO (Either WebDriverException a)
tryWebDriverException = try

-- | Throw a 'WebDriverException'
throwWebDriverException :: WebDriverException -> IO a
throwWebDriverException = throw

-- | Check if an exception is a protocol exception
isProtocolException :: WebDriverException -> Bool
isProtocolException = \case
  ProtocolException {} -> True
  _ -> False

-- | Check if an exception is a parse exception
isParseException :: WebDriverException -> Bool
isParseException = \case
  ResponseParseException {} -> True
  _ -> False

-- | Get the error type from a WebDriverException if it's a ProtocolException
getErrorType :: WebDriverException -> Maybe ErrorType
getErrorType = \case
  ProtocolException {error} -> Just error
  _ -> Nothing
