...
-- >>> runDemo inputEventFileDialogOpenedMultiple
inputEventFileDialogOpenedMultiple :: BiDiDemo
inputEventFileDialogOpenedMultiple =
  demo "Input Events - File Dialog Opened (Multiple Files)" action
  where
    action :: DemoUtils -> BiDiActions -> IO ()
    action utils@MkDemoUtils {..} cmds@MkCommands {..} = do
      logTxt "Subscribe to FileDialogOpened event"
      ...
      bc <- newWindowContext utils cmds
      logTxt "Navigating to upload page with multiple file input"
      url <- uploadUrl
      browsingContextNavigate $ MkNavigate bc url Nothing

      logTxt "Locating multiple file upload input"
      uploadInput <-
        browsingContextLocateNodes $
          MkLocateNodes
            { context = bc,
      ...
      logShow "Multiple upload input element" uploadInput

      let MkLocateNodesResult nodes = uploadInput
          uploadInputId :: Core.SharedId
          uploadInputId = case nodes of
            [input] -> fromJust input.sharedId
            _ -> error "Failed to locate multiple file upload input"

      logTxt "Clicking on multiple file upload input to open file dialog"
      logTxt "This should show multiple=true in the event"
      inputPerformActions $
        MkPerformActions
          { context = bc,
            actions =
      ...
      logTxt "Waiting for file dialog opened event (with multiple=true)..."
      waitFileDialogEventFired
